...
CREATE OR REPLACE FUNCTION add_enrollment() RETURNS TRIGGER LANGUAGE PLPGSQL AS
    ...
    declare el integer;
        BEGIN
	        execute format('CREATE OR REPLACE VIEW enrollment_section AS SELECT count(*) AS e_count from enroll e where e.section_id = %L and e.class_title = %L and e.quarter = %L and e.year = %L', new.section_id, new.class_title, new.quarter, new.year);
			SELECT s.enrollment_limit into el FROM section s WHERE s.course_id = new.course_id AND s.class_title = new.class_title AND s.section_id = new.section_id AND s.quarter = new.quarter AND s.year = new.year; 
			

            IF (SELECT e_count FROM enrollment_section) < el
                THEN RETURN NEW;
            ELSE
...
insert into enroll VALUES ('111111111', 1, 'CSE132A-1', 'SP', 2022, 'C00', 4, 'letter', 'enroll', 'IN');
