		...
		<tr>
			<td>
				...
				if (action != null && action.equals("select-report-I-c")) {
						...
						PreparedStatement cumulativeStmt = conn.prepareStatement("WITH classes_taken AS (SELECT c.*, e.grade, e.units_taken FROM enroll e, student s, classes c WHERE s.ssn = ? AND s.ssn = e.ssn AND c.class_title = e.class_title ORDER BY e.quarter, e.year) SELECT SUM(g.number_grade * c.units_taken)/SUM(c.units_taken) AS average FROM classes_taken c, grade_conversion g WHERE c.grade = g.letter_grade;");
